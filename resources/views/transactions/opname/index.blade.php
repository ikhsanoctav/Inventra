...
<div class="space-y-6" x-data="{ showModal: false }">
    ...
    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800 font-headline">Stock Opname</h2>
            <p class="text-sm text-slate-500">Penyesuaian stok sistem dengan jumlah fisik aktual di gudang.</p>
        </div>
        <button @click="showModal = true" class="px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-xl hover:bg-indigo-700 transition-colors shadow-sm flex items-center gap-2">
            <span class="material-symbols-outlined text-[18px]">add</span> Mulai Sesi Opname
        </button>
    </div>

    <!-- Filter Bar -->
    <div class="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm relative">
        <form action="{{ route('transactions.opname') }}" method="GET" class="flex flex-col sm:flex-row gap-4 relative"
              hx-get="{{ route('transactions.opname') }}"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('transactions.opname') }}" class="h-10 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold flex items-center transition-colors">
                    Reset
                </a>
                @endif
            </div>
            ...
            <table class="w-full text-left text-sm text-slate-600">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-6 py-4 font-mono font-medium text-slate-800">{{ $op->ref_number }}</td>
                        <td class="px-6 py-4">{{ $op->transaction_date }}</td>
                        <td class="px-6 py-4">{{ $op->user->name ?? 'System' }}</td>
                        <td class="px-6 py-4">
                            @if($op->status === 'Draft')
                        ...
                        <td class="px-6 py-4 text-center">
                            <a href="{{ route('transactions.opname.show', $op->id) }}" class="inline-flex items-center justify-center w-8 h-8 rounded-lg text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-colors">
                                <span class="material-symbols-outlined text-[18px]">{{ $op->status === 'Draft' ? 'edit' : 'visibility' }}</span>
                            </a>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-slate-500">
                            Belum ada riwayat pelaksanaan Stock Opname.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal" x-transition.scale.origin.bottom class="relative z-10 inline-block w-full max-w-md px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:p-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-slate-800">Mulai Sesi Hitung Fisik</h3>
                ...
                <form action="{{ route('transactions.opname') }}" method="POST">
                    @csrf
                    <div class="space-y-4 mb-6">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Nomor Dokumen</label>
                            <input type="text" name="ref_number" value="OPN-{{ date('Ymd-His') }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-indigo-500 bg-slate-50" readonly required>
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-3">
                        ...
                        <button type="submit" class="px-4 py-2 text-sm text-white bg-indigo-600 rounded-xl hover:bg-indigo-700">Mulai Opname</button>
                    </div>
                </form>
            </div>
        </div>
