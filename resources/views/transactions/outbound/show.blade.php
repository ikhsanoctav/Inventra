...
@section('header_title', 'Detail Outbound')

@section('main_content')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="font-headline text-2xl font-bold text-[#111827]">Detail Outbound: <span class="text-[#4F46E5]">{{ $transaction->ref_number }}</span></h2>
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('transactions.outbound') }}" class="h-10 px-4 rounded-lg bg-white border border-[#D1D5DB] hover:bg-gray-50 text-[#374151] text-sm font-semibold flex items-center gap-2 transition-all">
                <span class="material-symbols-outlined text-[18px]">arrow_back</span> Kembali
            </a>
            @if($transaction->status === 'Draft')
            <form action="{{ route('transactions.outbound.complete', $transaction->id) }}" method="POST">
                @csrf
                <button type="submit" class="h-10 px-4 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold flex items-center gap-2 shadow-sm transition-all" onclick="return confirm('Selesaikan transaksi? Stok akan dikurangi.')">
...
<table class="w-full text-left text-sm text-[#4B5563]">
                    ...
                    <tbody class="divide-y divide-[#E5E7EB]">
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 text-right">
                                @if($transaction->status === 'Draft')
                                <form action="{{ route('transactions.outbound.lines.destroy', $line->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-[#E5E7EB] shadow-sm overflow-hidden p-6">
                ...
                <form action="{{ route('transactions.outbound.addLine', $transaction->id) }}" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <select name="item_id" required class="w-full h-10 px-3 text-sm bg-white border border-[#D1D5DB] rounded-lg focus:outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20">
                            ...
                            <option value="{{ $item->id }}">{{ $item->sku }} - {{ $item->name }} (Stok: {{ $item->stock }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
