...
<div class="space-y-6" x-data="{ showModal: false }">
    ...
    <div class="bg-white p-4 rounded-2xl border border-[#E5E7EB] shadow-sm relative">
        <form action="{{ route('transactions.inbound') }}" method="GET" class="flex flex-col sm:flex-row gap-4 relative"
              hx-get="{{ route('transactions.inbound') }}"
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('transactions.inbound') }}" class="h-10 px-4 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-semibold flex items-center transition-colors">
                    Reset
                </a>
                @endif
            </div>
...
<table class="w-full text-left text-sm text-[#4B5563]">
            ...
            <tbody class="divide-y divide-[#E5E7EB]">
                @forelse($transactions as $inbound)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 font-mono font-bold text-indigo-600">{{ $inbound->ref_number }}</td>
                    <td class="px-6 py-4 text-[#6B7280]">{{ \Carbon\Carbon::parse($inbound->transaction_date)->format('d M Y') }}</td>
                    <td class="px-6 py-4 text-[#6B7280]">{{ $inbound->user->name ?? 'Sistem' }}</td>
                    <td class="px-6 py-4 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $inbound->status === 'Completed' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800' }}">
                            {{ $inbound->status }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <a href="{{ route('transactions.inbound.show', $inbound->id) }}" class="text-[#4F46E5] hover:text-[#4338CA] font-medium text-xs">Detail</a>
                        <form action="{{ route('transactions.inbound.destroy', $inbound->id) }}" method="POST" class="inline ml-2">
                            @csrf
                            @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-slate-500 font-medium">Tidak ada transaksi inbound yang ditemukan.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
            ...
            <div x-show="showModal" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-[#E5E7EB] z-10">
                <div class="bg-white px-6 pb-6 pt-6">
                    <div class="flex items-center justify-between mb-5 border-b border-[#E5E7EB] pb-4">
                        <h3 class="text-lg font-headline font-bold text-[#111827]">Buat Transaksi Inbound Baru</h3>
                    ...
                    <form action="{{ route('transactions.inbound') }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-semibold text-[#374151] mb-1.5">No. Referensi <span class="text-red-500">*</span></label>
                            <input type="text" name="ref_number" value="INB-{{ time() }}" required class="w-full h-10 px-3 text-sm bg-white border border-[#D1D5DB] rounded-lg focus:outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-[#374151] mb-1.5">Supplier (Opsional)</label>
                            <select name="supplier_id" class="w-full h-10 px-3 text-sm bg-white border border-[#D1D5DB] rounded-lg focus:outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/20">
